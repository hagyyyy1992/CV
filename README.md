...
<a href="https://cakephp.org/"><img src="https://img.shields.io/badge/CakePHP-D33C43?style=flat-square&logo=cakephp&logoColor=white" alt="CakePHP" /></a>
...
<a href="https://qdrant.tech/"><img src="https://img.shields.io/badge/Qdrant-DC244C?style=flat-square&logo=qdrant&logoColor=white" alt="Qdrant" /></a>
...
<a href="https://docs.anthropic.com/en/docs/claude-code"><img src="https://img.shields.io/badge/Claude_Code-191919?style=flat-square&logo=anthropic&logoColor=white" alt="Claude Code" /></a>
<a href="https://git-scm.com/docs/git-worktree"><img src="https://img.shields.io/badge/Git_Worktree-F05032?style=flat-square&logo=git&logoColor=white" alt="Git Worktree" /></a>
...
より具体的な条件・単価感はソースコードのコメントアウトの方に記載しました。

<!--  
  希望単価: 時給8,000円 or 月768,000円（週3日）
  下限単価: 時給6,000円 or 月576,000円（週3日）
  ※基本はスキル見合いでご予算感踏まえて前向きにすり合わせできればと思います。

  稼働:
  - 既存案件: 週2日（準委任契約）
  - 新規案件に割ける稼働: 週3日（平日）
  - 1日8h, 週3日（月約96h）稼働を想定。土日平日早朝夜間フレックスで週5稼働も可能です。

  根拠:
  - 現案件単価: 時給8,000円
  - 直近3年単価: 6,000円
  - AI駆動開発: Claude Code活用で1人で数十万行を安全に書き換え運用した実績（従来比5〜10倍）
...
- 業務内容
  ...
  - セキュリティ監査・改善（OWASP Top 10準拠・個人情報保護法対応・NoSQLインジェクション対策等）
...
- 業務内容
  - 介護医療事業所向けの請求管理・事務効率化サービスの開発支援
    ...
    - 入金情報を登録する機能
      - csv/txtアップロード
      - 全銀フォーマットの解析など
    - イベント管理機能の要件定義〜全工程担当（カレンダー・参加者管理・請求連携）
    - パフォーマンス改善
      - N+1問題解消
...
- 業務内容
  ...
  - DDDに基づくドメインモデルのリファクタリング（Value Object導入）
...
- 業務内容
  - バックエンドAPIを用いた新規画面UIの構築
  - 通知機能の設計・実装（未読バッジ表示・既読管理・通知一覧）
  - DatePickerコンポーネント実装（carbon-components-svelte）
  - 全APIへの認証ヘッダー（Authorization）横展開
  - タスク追加/削除機能の設計・実装（詳細画面）
- 使用技術、言語、フレームワーク
  - Svelte
  - Svelte Kit
  - typescript
...
- 業務内容
  - Slack連携して通知できる機能
  - コミュニティ機能（投稿タグ・フォロー・コメント承認・投稿承認/却下）の設計・実装
  - アンケート設問カスタマイズ機能の開発（フロント・バックエンド）
  - ライターアカウント機能の追加
  - コミュニティ機能の表示・非表示設定（Admin）
  - CSVレポート機能（ユーザー毎の動画再生数・会員登録日時）
  - 動画プレイヤー埋め込みエディタ機能（各メディアの埋め込み・添付の切り分け）
...
- 業務内容
  ...
  - S3バケット分離に伴うURL処理リファクタリング（テストコード付き）
- 使用技術、言語、フレームワーク
    - PHP 8.0
    - CakePHP 2系
    - Vue.js 2系
